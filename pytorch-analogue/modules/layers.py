...
class Linear(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        """
        return input @ self.weight.T + (0 if self.bias is None else self.bias)
...
class BatchNormalization(Module):
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        dx_hat = grad_output
        if self.affine:
            dx_hat = dx_hat * self.weight
        if self.training:
            B = input.shape[0]
            return (1. / B) * self.inv_sqrt_var * (B * dx_hat - np.sum(dx_hat, axis=0) - self.norm_input * np.sum(dx_hat * self.norm_input, axis=0))
        else:
            return dx_hat / np.sqrt(self.running_var + self.eps)
...
class Sequential(Module):
    ...
    def compute_output(self, input: np.array) -> np.array:
        ...
        for module in self.modules:
            input = module.compute_output(self.inputs[-1])
    ...
    def compute_grad_input(self, input: np.array, grad_output: np.array) -> np.array:
        ...
        # inputs = [input]
        # for module in self.modules:
        #     input = module.compute_output(inputs[-1])
        #     inputs.append(input)
        if self.training:
            for i in range(len(self.modules) - 1, -1, -1):
                prev_grad = grad_output
                grad_output = self.modules[i].compute_grad_input(self.inputs[i], prev_grad)
                self.modules[i].update_grad_parameters(self.inputs[i], prev_grad)
            return grad_output
